...
(* replaces #if defined(MACRO) by #ifdef MACRO *)
let repl_ifdef =
  List.map (fun (i,_line) ->
    let line = strip _line in
    let pat = Str.regexp "[#][ \t]*if[ \t]+defined[ \t]*[(]\\([^)]*\\)[)]" in
    if Str.string_match pat line 0 then
      let macro = Str.matched_group 1 line in
      let line = Printf.sprintf "#ifdef %s" macro in
      (i,line)
    else
      (i,_line)
  ) ;;
